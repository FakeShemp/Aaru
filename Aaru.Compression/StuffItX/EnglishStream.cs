...
public partial class EnglishStream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public EnglishStream(Stream compressedStream, long decompressedLength)
    {
        ...
        int err = AARU_stuffitx_english_decode_buffer(_decoded, ref outLen, inBuf, inBuf.Length);

        if(err != 0) throw new InvalidOperationException("English decompression failed");
    ...
    public static partial int AARU_stuffitx_english_decode_buffer(byte[] dst_buffer, ref nint dst_size,
