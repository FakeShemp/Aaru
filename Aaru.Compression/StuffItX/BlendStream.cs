...
namespace Aaru.Compression.StuffItX;

public partial class BlendStream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public BlendStream(Stream compressedStream, long decompressedLength)
    {
        ...
        int err = AARU_stuffitx_blend_decode_buffer(_decoded, ref outLen, inBuf, inBuf.Length);

        if(err != 0) throw new InvalidOperationException("Blend decompression failed");
    ...
    public static partial int AARU_stuffitx_blend_decode_buffer(byte[] dst_buffer, ref nint dst_size, byte[] src_buffer,
                                                                nint   src_size);
