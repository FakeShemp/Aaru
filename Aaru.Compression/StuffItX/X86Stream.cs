...
public partial class X86Stream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public X86Stream(Stream compressedStream, long decompressedLength)
    {
        ...
        int err = AARU_stuffitx_x86_decode_buffer(_decoded, ref outLen, inBuf, inBuf.Length);

        if(err != 0) throw new InvalidOperationException("X86 decompression failed");
    ...
    public static partial int AARU_stuffitx_x86_decode_buffer(byte[] dst_buffer, ref nint dst_size, byte[] src_buffer,
                                                              nint   src_size);
