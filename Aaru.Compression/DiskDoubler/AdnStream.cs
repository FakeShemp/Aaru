...
public partial class AdnStream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public AdnStream(Stream compressedStream, long decompressedLength)
    {
        ...
        int err = AARU_dd_adn_decode_buffer(_decoded, ref outLen, inBuf, inBuf.Length);

        if(err != 0) throw new InvalidOperationException("ADn decompression failed");
    ...
    public static partial int AARU_dd_adn_decode_buffer(byte[] dst_buffer, ref nint dst_size, byte[] src_buffer,
                                                        nint   src_size);
