...
public partial class JpegStream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public JpegStream(Stream compressedStream, long decompressedLength)
    {
        ...
        int err = AARU_zip_winzipjpeg_decode_buffer(_decoded, ref outLen, inBuf, inBuf.Length);

        if(err != 0) throw new InvalidOperationException("JPEG decompression failed");
    ...
    public static partial int AARU_zip_winzipjpeg_decode_buffer(byte[] dst_buffer, ref nint dst_size, byte[] src_buffer,
                                                                nint   src_size);
