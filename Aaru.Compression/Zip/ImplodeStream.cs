...
public partial class ImplodeStream : Stream
{
    readonly byte[] _decoded;
    readonly long   _length;
    long            _position;

    public ImplodeStream(Stream compressedStream, long decompressedLength, bool largeDictionary, bool hasLiterals)
    {
        ...
        int err = AARU_zip_implode_decode_buffer(_decoded,
                                                 ref outLen,
                                                 inBuf,
                                                 inBuf.Length,
                                                 largeDictionary ? 1 : 0,
        ...
        if(err != 0) throw new InvalidOperationException("Implode decompression failed");
    ...
    public static partial int AARU_zip_implode_decode_buffer(byte[] dst_buffer, ref nint dst_size, byte[] src_buffer,
                                                             nint   src_size,   int large_dictionary, int has_literals);
