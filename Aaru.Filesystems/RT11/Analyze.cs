...
//
// Component      : RT-11 filesystem plugin.
//
// --[ License ] --------------------------------------------------------------
//
...
#region IReadOnlyFilesystem Members
                                                   ...
                                                   EndProgressHandler                    endProgress    = null)
    {
        ...
        try
        {
            ...
            for(var i = 0; i < orderedEntries.Length; i++)
            {
                ...
                ErrorNumber errno = GetFileStartBlockAndLength(orderedEntries[i],
                                                               out uint startBlock,
                                                               out uint lengthInBlocks);

                if(errno == ErrorNumber.NoSuchFile) continue;

                if(errno != ErrorNumber.NoError) return errno;

                AddOverlappingFile(entryPath, startBlock, lengthInBlocks, normalizedExtents, files);
            }
    ...
    void AddOverlappingFile(string                                  path,          uint startBlock, uint lengthInBlocks,
                            IReadOnlyList<(ulong Start, ulong End)> sectorExtents, List<FileSectorInfo> files)
    {
        if(lengthInBlocks == 0) return;

        List<(ulong Start, ulong End)> overlaps = FindOverlappingExtents(startBlock, lengthInBlocks, sectorExtents);
    ...
    List<(ulong Start, ulong End)> FindOverlappingExtents(uint startBlock, uint lengthInBlocks,
                                                          IReadOnlyList<(ulong Start, ulong End)> sectorExtents)
    {
        List<(ulong Start, ulong End)> overlaps = [];

        ulong startSector = _partition.Start             + startBlock;
        ulong endSector   = startSector + lengthInBlocks - 1;
