...
#region IReadOnlyFilesystem Members
                                   ...
                                   List<FileSectorInfo> files)
    {
        ...
        if((inode.di_mode & S_IFMT) == S_IFDIR) return ErrorNumber.NoError;
        if(inode.di_size            <= 0) return ErrorNumber.NoError;

        errno = FindOverlappingExtents(inodeNumber, inode, sectorExtents, out List<(ulong Start, ulong End)> overlaps);
    ...
    ErrorNumber FindOverlappingExtents(ulong                                   inodeNumber, Dinode inode,
                                       IReadOnlyList<(ulong Start, ulong End)> sectorExtents,
                                       out List<(ulong Start, ulong End)>      overlaps)
    {
        overlaps = [];

        if(inode.di_size <= 0 || inode.di_format == XFS_DINODE_FMT_LOCAL) return ErrorNumber.NoError;

        ErrorNumber errno = LoadFileExtents(inodeNumber, inode, out XfsExtent[] extents);

        if(errno != ErrorNumber.NoError) return errno;

        var   logicalSize = (ulong)inode.di_size;
        ulong blockSize   = _superblock.blocksize;

        foreach(XfsExtent extent in extents)
        {
            if(extent.BlockCount == 0 || extent.Unwritten) continue;

            ulong logicalOffset = extent.StartOff * blockSize;

            if(logicalOffset >= logicalSize) continue;

            ulong extentBytes = Math.Min(extent.BlockCount * blockSize, logicalSize - logicalOffset);
            AddByteRangeOverlaps(extent.StartBlock * blockSize, extentBytes, sectorExtents, overlaps);
        }
