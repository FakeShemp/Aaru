...
public sealed partial class LIF
{
                             ...
                             Dictionary<string, string> options,     string    @namespace)
    {
        ...
        if(_systemBlock.magic != LIF_MAGIC)
            return ErrorNumber.InvalidArgument;

        // Validate directory parameters
        if(_systemBlock.directoryStart == 0 || _systemBlock.directorySize == 0)
            return ErrorNumber.InvalidArgument;
        ...
        for(int offset = 0; offset + directoryEntrySize <= directoryData.Length; offset += directoryEntrySize)
        {
