...
{
    /// <summary>Mount a GameCube disc filesystem (no encryption)</summary>
    ...
    {
        // Find the first data partition (type 0)
        var  partCount1 = BigEndianBitConverter.ToUInt32(discData, 0x40000);
        uint partOff1   = BigEndianBitConverter.ToUInt32(discData, 0x40004) << 2;

        WiiPartitionTableEntry? dataPartition = null;

        for(var i = 0; i < partCount1; i++)
        {
            if(partOff1 + i * 8 + 8 >= 0x50000) continue;

            WiiPartitionTableEntry entry =
                Marshal.ByteArrayToStructureBigEndian<WiiPartitionTableEntry>(discData, (int)(partOff1 + i * 8), 8);
        ...
        // If no data partition in first table, check others
        if(!dataPartition.HasValue)
        {
            uint[] partCounts  = [partCount1];
            uint[] partOffsets = [partOff1];

            for(var tbl = 1; tbl < 4 && !dataPartition.HasValue; tbl++)
            {
                ...
                for(var i = 0; i < count; i++)
                {
                    ...
                    WiiPartitionTableEntry entry =
                        Marshal.ByteArrayToStructureBigEndian<WiiPartitionTableEntry>(discData,
                            (int)(offset + i * 8),
                            8);
        ...
        if(!dataPartition.HasValue)
        {
            AaruLogging.Debug(MODULE_NAME, "No data partition found on Wii disc");

            return ErrorNumber.InvalidArgument;
        }

        _partitionOffset = dataPartition.Value.Offset;

        AaruLogging.Debug(MODULE_NAME, "Wii data partition at offset 0x{0:X8}", _partitionOffset);
        ...
        ulong ticketSector = (_partitionOffset + 0x1BF) / _imagePlugin.Info.SectorSize;

        var ticketSectorCount =
            (uint)((_partitionOffset +
                    0x1DC            +
                    16 -
        ...
        var ticketBase = (uint)(_partitionOffset + 0x1BF - ticketSector * _imagePlugin.Info.SectorSize);
        ...
        ulong keyIndexSector = (_partitionOffset + 0x1F1) / _imagePlugin.Info.SectorSize;

        errno = _imagePlugin.ReadSectors(keyIndexSector, false, 1, out byte[] keyIndexData, out _);

        if(errno != ErrorNumber.NoError) return errno;

        byte keyIndex = keyIndexData[(_partitionOffset + 0x1F1) % _imagePlugin.Info.SectorSize];
        ...
        AaruLogging.Debug(MODULE_NAME, "Decrypted partition key: {0}", BitConverter.ToString(partitionKey));

        // Set up AES for partition data decryption
        _partitionAes         = Aes.Create();
        _partitionAes.Key     = partitionKey;
        _partitionAes.Mode    = CipherMode.CBC;
        _partitionAes.Padding = PaddingMode.None;

        // Read data offset and data size from the partition header at partition offset + 0x2B8
        ulong dataOffSector = (_partitionOffset + 0x2B8) / _imagePlugin.Info.SectorSize;

        errno = _imagePlugin.ReadSectors(dataOffSector, false, 1, out byte[] dataOffData, out _);

        if(errno != ErrorNumber.NoError) return errno;

        var dataOffBase = (uint)((_partitionOffset + 0x2B8) % _imagePlugin.Info.SectorSize);

        _partitionDataOffset = (ulong)BigEndianBitConverter.ToUInt32(dataOffData, (int)dataOffBase) << 2;

        AaruLogging.Debug(MODULE_NAME, "Wii partition data offset: 0x{0:X8}", _partitionDataOffset);

        // Read the partition's internal disc header to get FST offset and size
        // These are at offset 0 within the partition data, within the encrypted region
        byte[] partHeader = ReadWiiPartitionData(0, 0x440);

        if(partHeader == null)
        {
            AaruLogging.Debug(MODULE_NAME, "Failed to read Wii partition header");

            return ErrorNumber.InvalidArgument;
        }

        DiscHeader partDiscHeader = Marshal.ByteArrayToStructureBigEndian<DiscHeader>(partHeader);

        // Wii partition internal FST offset and size need to be shifted left by 2
        uint fstOff  = partDiscHeader.FstOff  << 2;
        uint fstSize = partDiscHeader.FstSize << 2;

        AaruLogging.Debug(MODULE_NAME, "Wii partition FST offset: 0x{0:X8}, size: 0x{1:X8}", fstOff, fstSize);

        if(fstOff == 0 || fstSize == 0) return ErrorNumber.InvalidArgument;

        // Read FST from the encrypted partition data
        byte[] fstData = ReadWiiPartitionData(fstOff, fstSize);

        if(fstData == null)
        {
            AaruLogging.Debug(MODULE_NAME, "Failed to read Wii FST");
    ...
    byte[] ReadWiiPartitionData(uint offset, uint size)
    {
        ...
        while(bytesRead < size)
        {
            ...
            ulong physicalOffset = _partitionOffset + _partitionDataOffset + (ulong)blockIndex * WII_CLUSTER_SIZE;
            ...
            _partitionAes.IV = dataIv;

            byte[] decryptedData;

            using(ICryptoTransform decryptor = _partitionAes.CreateDecryptor())
            {
    ...
    /// <param name="isWii">
    ///     True if Wii (file offsets need <<2), false for
    ///                                                GameCube
    /// </param>
    /// <returns>Error number indicating success or failure</returns>
    ErrorNumber ParseFst(byte[] fstData, bool isWii)
    {
        ...
        // Parse all entries
        _fstEntries = new FstEntry[totalEntries];
        _fstNames   = new string[totalEntries];

        _fstEntries[0] = rootEntry;
        _fstNames[0]   = "";

        for(uint i = 1; i < totalEntries; i++)
        {
            _fstEntries[i] = Marshal.ByteArrayToStructureBigEndian<FstEntry>(fstData, (int)(i * 12), 12);

            uint nameOffset = _fstEntries[i].TypeAndNameOffset & 0x00FFFFFF;
            _fstNames[i] = StringHandlers.CToString(fstData, _encoding, false, (int)(stringTableOffset + nameOffset));

            AaruLogging.Debug(MODULE_NAME,
                              "ParseFst: entry[{0}] type={1} name=\"{2}\" offsetOrParent=0x{3:X8} sizeOrNext=0x{4:X8}",
                              i,
                              _fstEntries[i].TypeAndNameOffset >> 24 != 0 ? "dir" : "file",
                              _fstNames[i],
                              _fstEntries[i].OffsetOrParent,
                              _fstEntries[i].SizeOrNext);
        }
    ...
    /// <summary>Cache the root directory contents (entries whose parent is 0)</summary>
    void CacheRootDirectory()
    {
        ...
        for(var i = 1; i < _fstEntries.Length; i++)
        {
            ...
            if(isDirectory)
            {
                ...
                if(_fstEntries[i].OffsetOrParent == 0) _rootDirectoryCache[_fstNames[i]] = i;

                // Skip children of non-root directories by not adding them here
                // (they belong to their parent directory, not root)
            }
            else
            {
                // For files, we need to check if they're in the root directory
                // A file is in the root if it's between indices 1 and root.SizeOrNext,
                // and not inside any subdirectory range
                if(IsDirectChild(i, 0)) _rootDirectoryCache[_fstNames[i]] = i;
            }
        }
    }

    /// <summary>Check if FST entry at index is a direct child of the directory at parentIndex</summary>
    bool IsDirectChild(int entryIndex, int parentIndex)
    {
        // For files: walk backwards to find the innermost directory that contains this entry
        // For directories: simply check OffsetOrParent

        bool isDirectory = _fstEntries[entryIndex].TypeAndNameOffset >> 24 != 0;

        if(isDirectory) return (int)_fstEntries[entryIndex].OffsetOrParent == parentIndex;

        // For a file, find the innermost enclosing directory
        // Walk the directory entries to find which directory this file belongs to
        var currentParent = 0;

        for(var i = 0; i < _fstEntries.Length; i++)
        {
            if(i == entryIndex) return currentParent == parentIndex;

            bool isDir = _fstEntries[i].TypeAndNameOffset >> 24 != 0;
            ...
            if(entryIndex > i && entryIndex < (int)_fstEntries[i].SizeOrNext) currentParent = i;
        }
