...
#region IReadOnlyFilesystem Members
    ...
    ErrorNumber TraverseDirectoryForAffectedSectors(string path, qnx6_inode_entry directoryInode,
                                                    ...
                                                    PulseProgressHandler pulseProgress)
    {
        ...
        ErrorNumber errno = GetDirectoryEntries(path, directoryInode, out Dictionary<string, qnx6_inode_entry> entries);

        if(errno != ErrorNumber.NoError) return errno;

        KeyValuePair<string, qnx6_inode_entry>[] orderedEntries = entries
                                                                 .Where(static entry =>
        ...
        for(var i = 0; i < orderedEntries.Length; i++)
        {
            ...
            qnx6_inode_entry entry     = orderedEntries[i].Value;
    ...
    ErrorNumber GetDirectoryEntries(string                                   path, qnx6_inode_entry directoryInode,
                                    out Dictionary<string, qnx6_inode_entry> entries)
    {
        if(path == "/")
        {
            entries = new Dictionary<string, qnx6_inode_entry>(_rootDirectoryCache, StringComparer.Ordinal);
    ...
    ErrorNumber AddOverlappingFile(string                                  path,          qnx6_inode_entry     inode,
                                   IReadOnlyList<(ulong Start, ulong End)> sectorExtents, List<FileSectorInfo> files)
    {
        ...
        {
            Path            = path,
            Inode           = GetInodeIdentifier(inode),
    ...
    static ulong GetInodeIdentifier(qnx6_inode_entry inode)
    {
        if(inode.di_block_ptr is { Length: > 0 })
        {
            for(var i = 0; i < inode.di_block_ptr.Length; i++)
            {
                if(inode.di_block_ptr[i] == 0) continue;

                return (ulong)inode.di_block_ptr[i] << 32 | (uint)Math.Min(inode.di_size, uint.MaxValue);
            }
        }
    ...
    ErrorNumber FindOverlappingExtents(qnx6_inode_entry inode, IReadOnlyList<(ulong Start, ulong End)> sectorExtents,
                                       out List<(ulong Start, ulong End)> overlaps)
    {
        ...
        var   totalBlocks    = (uint)((inode.di_size + _blockSize - 1) / _blockSize);
        ...
        for(uint logicalBlock = 0; logicalBlock < totalBlocks && remainingBytes > 0; logicalBlock++)
        {
            ...
            ulong byteOffset      = (ulong)physicalBlock * _blockSize;
