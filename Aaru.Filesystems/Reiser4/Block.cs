...
public sealed partial class Reiser4
{
    ...
    ErrorNumber ReadBlock(ulong blockNumber, out byte[] data)
    {
        data = null;
        ...
        ulong byteOffset = blockNumber * _blockSize;

        // Calculate which sector contains this byte offset
        ulong sectorNumber   = byteOffset / sectorSize + _partition.Start;
        ...
        else
        {
            ...
            Array.Copy(sectorData,
                       offsetInSector,
                       data,
                       0,
                       (long)Math.Min(_blockSize, (ulong)(sectorData.Length - offsetInSector)));
        }
