...
#region IReadOnlyFilesystem Members
                                                    ...
                                                    PulseProgressHandler pulseProgress)
    {
        ...
        ErrorNumber errno = ReadInode(directoryInodeNumber, out ext_inode directoryInode);
        ...
        for(int i = 0; i < orderedEntries.Length; i++)
        {
            ...
            errno = ReadInode(orderedEntries[i].Value, out ext_inode inode);
    ...
    ErrorNumber GetDirectoryEntries(uint directoryInodeNumber, ext_inode directoryInode,
                                    out Dictionary<string, uint> directoryEntries)
    {
        if(directoryInodeNumber == EXT_ROOT_INO)
        {
        ...
        return ReadDirectoryEntries(directoryInode, out directoryEntries);
    }

    ErrorNumber AddOverlappingFile(string path, uint inodeNumber, ext_inode inode,
                                   IReadOnlyList<(ulong Start, ulong End)> sectorExtents,
                                   List<FileSectorInfo> files)
    {
    ...
    ErrorNumber FindOverlappingExtents(ext_inode inode, ulong logicalSize,
                                       IReadOnlyList<(ulong Start, ulong End)> sectorExtents,
                                       out List<(ulong Start, ulong End)> overlaps)
    {
        overlaps = [];

        ulong blockSize = EXT_BLOCK_SIZE;
        ulong blockCount = (logicalSize + blockSize - 1) / blockSize;

        for(uint logicalBlock = 0; logicalBlock < blockCount; logicalBlock++)
        {
